...
### 受け入れテスト後の修正（2026-09-25・合格判定を受けて）

1. **80字超の省略順**：**作品名を短縮 → ブランド → 版とスケール → 商品ライン**。
   先に作品名を縮めると、版やスケールを残したまま収まることが多い。
   商品ライン（Nendoroid / figma / S.H.Figuarts / POP UP PARADE 等）は買い手が
   検索に使うので最後まで残す。
2. AIの出力に **`series_short`** を追加（例 `Atelier Ryza 2`）。80字を超えるときに使う。
3. `ja_title` から **スケールをコードで抽出**し、`1/7 Scale` として英題に入れる。
   ノンスケールは入れない。省略の優先度は版と同じ。
4. **魂ネイションズ系のブランドは Bandai Spirits に正規化**
   （Tamashii Nations → Bandai Spirits。S.H.Figuarts・超合金も同じ）。
   Orange Rouge はそのまま。
5. **飾りハイフンを整理**（`Touken Ranbu -ONLINE-` → `Touken Ranbu Online`）。
   外した先が4文字以上の全部大文字なら頭だけ大文字に直す。
   語中のハイフン（Yu-Gi-Oh! など）は触らない。
...
1. **出品文の一括作成**：出品CSVタブに「出品文を一括作成（英題がある行）」を追加。
   出品文タブを開かずに説明文を作って行へ書き戻す。生成は出品文タブと同じ
   `descHTML()` に任せ、各欄を行の値で一時的に差し替えて呼び、必ず元へ戻す
   （編集中の内容を壊さない）。英題がある行のうち、説明文が空の行だけが対象。
2. **カテゴリの自動振り分け**：`CSV_LINE` に4列目（`act`=可動／`fig`=非可動）を持たせ、
   商品ラインとスケールからカテゴリを決める。
   figma / S.H.Figuarts / Nendoroid → 可動（261068）、
   POP UP PARADE / 一番くじ / Figuarts ZERO / スケールもの → 非可動（261055）。
   PROPLICA・超合金は決めず、「カテゴリ未振分」のまま残す。
3. **原産国**：SP-API（日本）のカタログ属性 `country_of_origin`（`country_as_labeled` /
   `item_origin` も見る）から取り、eBayの表記に正規化する（JP/日本 → Japan、
   CN/中国 → China ほか。`Made in Japan` のような書き方も拾う）。
   取れなかった行は Worker が空で返し、pj_price が設定の「原産国の既定値」
   （初期値 China）を入れる。

   **`status` は英題の信頼度だけで決める**（2026-09-26 改訂）。原産国が既定値なだけの行は
   `ok` のままにし、代わりに一覧に黄色のバッジ「原産国：既定値」を出す。
   Amazonから取れた行にはバッジを付けない。件数表示にも「原産国が既定値 n件」を出し、
   一覧の絞り込みで「原産国：既定値の行だけ」を選べる（ほかに「要確認（review）の行だけ」
   「不備がある行だけ」）。絞り込みは表示だけに効き、「次の未完成へ」の巡回には影響しない。

   ただし `item_type` が figure 以外の行は、英題の形（末尾の `Figure` と商品名の位置）が
   変わるため、英題の確認が要るものとして `review` に落とす。
4. **フィギュアでない商品**：AIの出力に `item_type`（figure / prop_replica / other）と
   `item_name` を追加。figure 以外は英題の末尾に `Figure` を付けず、
   商品名（Storage Bangle など）をキャラクター名の後に入れる。
   版が `Ver.` だけになる場合は空にする（Worker・pj_price の両方で落とす）。
